...
namespace monopoly {
	...
	public:
		...
		[[nodiscard]]
		constexpr CardType next_card() noexcept {
			auto const card = _cards[_top_index];
		...
		void shuffle(auto& random_engine) {
			...
			for (unsigned i = 0; i < Size; ++i) {
				auto const new_idx = random_engine() % Size;
				std::swap(_cards[i], _cards[new_idx]);
			}
		}

	private:
		std::array<CardType, Size> _cards;
		unsigned _top_index = 0;
