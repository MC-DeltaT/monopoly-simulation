...
namespace monopoly {
	...
	inline void advance_by_spaces(game_state_t& game_state, player_strategies_t& strategies, random_t& random,
			unsigned const player, unsigned const offset) {
	...
	inline void advance_by_spaces_no_go(game_state_t& game_state, player_strategies_t& strategies, random_t& random,
			unsigned player, unsigned const offset) {
		auto const passed_go = advance_position_relative(game_state, player, offset);
		assert(!passed_go);
	}
	...
	inline void retreat_by_spaces(game_state_t& game_state, player_strategies_t& strategies, random_t& random,
			unsigned const player, unsigned const offset) {
	...
	inline void go_to_jail(game_state_t& game_state, unsigned const player) {
		...
		++statistics.sent_to_jail_count[player];
	}
